...
def parse_args():
    ...
    parser.add_argument(
        "--job_name", default="convnext_ortho", type=str, help="Job name"
    )
    parser.add_argument(
        "--job_dir", default="", type=str, help="Job directory; leave empty for default"
    )
    parser.add_argument(
        "--partition", default="booster", type=str, help="Partition where to submit"
    )
    parser.add_argument(
        "--use_volta32", action="store_true", default=True, help="Big models? Use this"
    )
    parser.add_argument("--account", default="besl", type=str, help="Account name")
    parser.add_argument(
        "--comment",
        default="",
        type=str,
...
def get_shared_folder() -> Path:
    user = "/p/project/besl/"
    if Path("/checkpoint/ConvNeXt").is_dir():
